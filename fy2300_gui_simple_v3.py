...
def generar_builtin_preview(
    ...
    duty = max(0.1, min(99.9, duty_percent)) / 100.0
...
class FY2300Controller:
    """
    Control basado SOLO en fy2300_serial.py proporcionado por el usuario.
    Importante:
    - Este wrapper expone control del canal principal (CH1).
    - No se implementa CH2 porque no hay API separada visible en fy2300_serial.py.
    """
    ...
    @staticmethod
    def _validate_channel(channel_ui: int):
        if int(channel_ui) != 1:
            raise RuntimeError(
                "La versión actual de fy2300_serial.py que compartiste "
                "solo expone control directo del canal principal (CH1). "
                "CH2 no se configuró para no inventar comandos."
            )
    ...
    def test_connection(self):
        with self._open() as gen:
            info = {
                ...
                "has_set_frequency_hz": hasattr(gen, "set_frequency_hz"),
                "has_set_amplitude_vpp": hasattr(gen, "set_amplitude_vpp"),
                "has_set_offset_v": hasattr(gen, "set_offset_v"),
                "has_set_duty_percent": hasattr(gen, "set_duty_percent"),
                "has_set_attenuation": hasattr(gen, "set_attenuation"),
                "has_set_output": hasattr(gen, "set_output"),
            }
            return info
    ...
    def apply_builtin_wave(
        ...
        self._validate_channel(channel_ui)
        ...
        with self._open() as gen:
            ...
            # Muy importante para evitar el factor x0.1 por atenuación
            gen.set_attenuation(int(attenuation_mode))
            time.sleep(0.5)
            ...
            if str(wave_name).strip().lower() == "square":
                gen.set_duty_percent(float(duty_percent))
                time.sleep(0.5)
            ...
            return (
                f"Onda '{wave_name}' aplicada en CH1. "
                f"Atenuación={'0 dB' if int(attenuation_mode) == FY2300.ATTEN_0DB else '20 dB'}."
            )

    def set_output(self, enabled, channel_ui=1):
        self._validate_channel(channel_ui)

        with self._open() as gen:
            gen.set_output(bool(enabled))
            return f"Salida CH1 {'ON' if enabled else 'OFF'}."
    ...
    def upload_csv_segment_as_arb2(
        ...
        with self._open() as gen:
            ...
            # Igual que el script que sí te funcionó, añadiendo atenuación explícita.
            gen.set_output(False)
            time.sleep(1.0)
            ...
            info["result"] = result
            info["message"] = (
                "arb2 cargada y configurada en CH1 desde segmento CSV."
            )
            return info
...
class FY2300GuiApp:
    def __init__(self, root):
        self.root = root
        self.root.title("FY2300 GUI simple v3")
        ...
        self.attenuation_var = tk.StringVar(value="0 dB")
    ...
    def _build_ui(self):
        ...
        ttk.Combobox(
            ...
            values=["sin", "square", "triangle", "ramp_up", "gaussian_white_noise", "arb2"],
            state="readonly",
            width=18,
    ...
    def _attenuation_mode(self):
        return FY2300.ATTEN_0DB if self.attenuation_var.get() == "0 dB" else FY2300.ATTEN_20DB
    ...
    def _plot_builtin_preview(self):
        try:
            wave_name = self.wave_var.get().strip().lower()
            ...
            preview_name = {
                "triangle": "triangle",
                "ramp_up": "ramp_up",
                "gaussian_white_noise": "gauss",
            }.get(wave_name, wave_name)

            t, y = generar_builtin_preview(
                preview_name, freq_hz, amplitude_vpp, offset_v, duty_percent
            )
    ...
    def on_test_connection(self):
        def worker():
            ...
            return (
                ...
                f"upload_waveform={info['has_upload_waveform']} "
                f"wave={info['has_set_wave_builtin']} "
                f"freq={info['has_set_frequency_hz']} "
                f"amp={info['has_set_amplitude_vpp']} "
                f"offset={info['has_set_offset_v']} "
                f"duty={info['has_set_duty_percent']} "
                f"atten={info['has_set_attenuation']} "
                f"out={info['has_set_output']}"
            )
    ...
    def on_apply_builtin(self):
        def worker():
            ...
            offset_v = float(self.offset_var.get())
            duty_percent = float(self.duty_var.get())
            attenuation_mode = self._attenuation_mode()
    ...
    def on_set_output(self, enabled):
        def worker():
            channel_ui = int(self.channel_var.get())
            return self._controller().set_output(enabled, channel_ui=channel_ui)
